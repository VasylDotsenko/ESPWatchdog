...
Проєкт ще не є фінальним production-релізом. Поточний стан позначено як `v0.4.20-power-status-snapshot`.
...
1. Зафіксувати `v0.4.20-power-status-snapshot` як hardware-verified baseline.
