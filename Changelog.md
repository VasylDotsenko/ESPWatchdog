...
Hardware verification hotfix для роботи без користувачів.

### Виправлено

- змінено поведінку `WiFiService`, щоб пристрій не залишався назавжди в AP mode після втрати домашньої WiFi-мережі;
- якщо `wifi.ssid` заданий, але підключення до домашньої мережі не вдається, setup portal працює як тимчасовий rescue mode;
- після 5 хвилин у `ESP-Watchdog-Setup` пристрій вимикає AP і знову пробує підключитися до налаштованої WiFi-мережі;
- якщо нормальне WiFi-підключення не відновилося протягом 20 хвилин, ESP виконує контрольований restart;
- перший старт із порожнім `wifi.ssid` залишається безпечним: setup portal не перезавантажує ESP, щоб користувач міг виконати первинне налаштування.
...
- Watchdog / PowerService логіку;
- Web API contract.

---

## [0.9.1-hardware-verification] - 27.08.2026

### Статус

Почато Release Candidate hardware verification phase.

### Додано

- `HardwareVerification.md`;
- покроковий сценарій перевірки:
  - boot;
  - WiFi;
  - HealthCheck;
  - Watchdog / PowerService;
  - Tuya LAN `3.5`;
  - Web UI / API;
  - OTA;
  - diagnostics.
- expected logs для ключових сценаріїв;
- pass/fail критерії для переходу до `0.9.2`.

### Не змінено

- firmware feature set;
- Tuya relay command path;
- Web API behavior;
- config schema.
...
Додано базові метрики довготривалої роботи для контролю heap trend.

### Додано

- `RuntimeGuardStatus::heapAtBoot`;
- `RuntimeGuardStatus::minFreeHeapSeen`;
- `RuntimeGuardStatus::heapDropFromBoot`;
- `RuntimeGuardStatus::maxHeapFragmentationSeen`;
- `RuntimeGuardStatus::lastCheckAt`;
- нові поля у `/api/diagnostics`;
- картку `Runtime trend` на dashboard.

### Причина

Після стабілізації memory footprint важливо бачити не тільки поточний heap, а й тренд за час роботи. Це дозволяє швидко оцінювати 24h / 7d stability без ручного порівняння окремих JSON snapshot-ів.
